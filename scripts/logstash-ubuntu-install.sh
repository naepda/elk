...
setup_data_disk()
{
  ...
  mkdir -p "$RAIDDISK/data"
  chown -R elk4sa:elk4sa "$RAIDDISK"
  chmod -R 777 "$RAIDDISK"
}
...
install_logstash()
{
    ...
    log "[install_logstash] Downloaded logstash $ES_VERSION"
  	...
  	cp -a ./logstash ./cesa-selfserving
  	mv ./logstash ./blobdownload

  	mkdir -p ${RETURN_HOME}/blobdownload/install_gem
  	mkdir -p ${RETURN_HOME}/cesa-selfserving/install_gem

  	adduser logstash --disabled-password

  	chown elk4sa:elk4sa -R ./blobdownload
  	chown elk4sa:elk4sa -R ./cesa-selfserving
  	chmod 775 -R ./blobdownload
  	chmod 775 -R ./cesa-selfserving

  	chmod 777 -R ${RETURN_HOME}/blobdownload/install_gem/
  	chmod 777 -R ${RETURN_HOME}/cesa-selfserving/install_gem/
    ...
    sudo wget -q "$DOWNLOAD_GEM_URL" -O logstash-input-azureblobdownload-0.9.8.gem
  	...
  	cp ./logstash-input-azureblobdownload-0.9.8.gem ${RETURN_HOME}/cesa-selfserving/install_gem/
  	cp ./logstash-input-azureblobdownload-0.9.8.gem ${RETURN_HOME}/blobdownload/install_gem/


  	cd ${RETURN_HOME}/cesa-selfserving/bin
  	./logstash-plugin install logstash-input-azureblob
  	./logstash-plugin install logstash-filter-date_formatter
  	./logstash-plugin install ${RETURN_HOME}/cesa-selfserving/install_gem/logstash-input-azureblobdownload-0.9.8.gem

  	cd ${RETURN_HOME}/blobdownload/bin
  	cd ./bin
  	./logstash-plugin install logstash-input-azureblob
  	./logstash-plugin install logstash-filter-date_formatter
  	./logstash-plugin install ${RETURN_HOME}/blobdownload/install_gem/logstash-input-azureblobdownload-0.9.8.gem
